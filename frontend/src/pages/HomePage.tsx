...
export default function HomePage() {
  ...
  const hero = page?.hero_image_url || "https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=1600&q=80";

  return (
    <>
      <PageMeta title={page?.title || site.site_name} description={page?.meta_description} />
      <section className="hero">
        <div className="hero-bg" style={{ backgroundImage: `linear-gradient(105deg, rgba(28,25,23,0.75) 0%, rgba(44,74,62,0.55) 45%, rgba(28,25,23,0.35) 100%), url("${hero}")` }} />
        <div className="container hero-grid">
          ...
          <div className="hero-book"><LiveQuoteWidget /></div>
        </div>
      </section>
        ...
        <div className="container">
          ...
          <div className="experience-grid">
            <Link to="/stay" className="experience-card">
              <div className="experience-img" style={{ backgroundImage: `url("${gallery[3]?.url || gallery[0]?.url}")`, backgroundSize: "cover", backgroundPosition: "center" }} />
            ...
            <Link to="/events" className="experience-card">
              <div className="experience-img" style={{ backgroundImage: `url("${gallery[1]?.url}")`, backgroundSize: "cover", backgroundPosition: "center" }} />
              <div className="experience-body">
        ...
        <div className="container">
          ...
          <div className="gallery-grid">
            {gallery.slice(0, 6).map((g) => (
              <div key={g.id} className="gallery-tile" style={{ backgroundImage: `url("${g.url}")`, backgroundSize: "cover", backgroundPosition: "center" }}>
                <span>{g.title}</span>
              </div>
            ))}
          </div>
      ...
      <section className="trust-strip">
        <div className="container trust-inner">
          {site.trust_items.map((t) => <span key={t}>{t}</span>)}
        </div>
      </section>

      <style>{`
        .hero { position: relative; min-height: 92vh; display: flex; align-items: center; padding: 6rem 0 4rem; color: #fff; }
        ...
        .hero-copy h1 { font-size: clamp(2.25rem, 5vw, 3.25rem); color: #fff; margin: 0.75rem 0 1.25rem; max-width: 16ch; }
        .hero-copy p { font-size: 1.05rem; line-height: 1.7; opacity: 0.9; max-width: 42ch; margin-bottom: 2rem; }
        ...
        .highlights-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 1px; background: var(--border); border-radius: var(--radius-lg); overflow: hidden; box-shadow: var(--shadow); }
        .highlight-item { background: var(--surface); padding: 1.5rem 1.25rem; text-align: center; }
        .highlight-item strong { display: block; font-family: var(--font-display); font-size: 1.15rem; margin-bottom: 0.35rem; }
        ...
        .experience-card { display: block; background: var(--surface); border-radius: var(--radius-lg); overflow: hidden; border: 1px solid var(--border); color: inherit; transition: transform .2s; }
        ...
        .gallery-tile { aspect-ratio: 4/3; border-radius: var(--radius); display: flex; align-items: flex-end; padding: 1rem; color: #fff; font-size: 0.82rem; font-weight: 600; text-shadow: 0 1px 4px rgba(0,0,0,.5); }
        .trust-strip { padding: 2rem 0; border-top: 1px solid var(--border); }
        .trust-inner { display: flex; flex-wrap: wrap; justify-content: center; gap: 1rem 2rem; font-size: 0.78rem; font-weight: 600; text-transform: uppercase; color: var(--text-muted); }
        .trust-inner span::before { content: "✓ "; color: var(--green); }
        @media (max-width: 900px) {
          .hero-grid, .experience-grid, .gallery-grid { grid-template-columns: 1fr; }
